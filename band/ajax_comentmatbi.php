...
session_start();
	...
	if(!empty($_GET["opcao"])){
		...
		if ($_GET["opcao"] == "buscaTextoComentario"){
			$codMateriaBI = $_GET['codMateriaBI'];
			$materiaBi = $fachadaSist2->lerRegistroMateriaBI($codMateriaBI);
			$textoComentario = $materiaBi->getTextoComentario();
			echo $textoComentario;
		}
		
	}
	// Função para inserir comentário nas NBI - Sgt Bedin
if ($_POST["opcao"] == "enviaComentario"){
		$codMateriaBI = $_POST['codMateriaBI'];
		$textoNovo = $_POST['textoComentario'];
		$materiaBi = $fachadaSist2->lerRegistroMateriaBI($codMateriaBI);
		$textoAtual = $materiaBi->getTextoComentario();
		$usuario = $_SESSION['NOMEUSUARIO'];
		$dataAtual = date(" d/m/Y - H:i:s");
		
		if ($textoAtual == "") {
		$textoComentario = "Usuário: " . $usuario . " - " . $dataAtual . ": ". $textoNovo;
		$materiaBi = $fachadaSist2->alterarComentario($codMateriaBI, $textoComentario);
		}else{
		$textoComentario = "Usuário: " . $usuario . " - " . $dataAtual . ": ". $textoNovo . "<br>" . $textoAtual;
			//echo $textoComentario;
