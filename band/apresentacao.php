		...
		return '<script type="text/javascript">
			...
			function viewHelp(){
				if ((janelaHelp == undefined)){
					janelaHelp = window.open("help_sisbol/help_sisbol.htm","","height=500,width=900,resizable,toolbar,menubar");
			...
			function online(){
				if ((janelaOnline == undefined)){
					janelaOnline = window.open("http://10.12.4.3/sitesisbol/","","height=500,width=900,resizable,toolbar,scrollbars,location");
...
//		print_r($this->aSubArq);
		...
		//varre o vetor da barra de menu principal
  		for ($i=1; $i <= count($this->aMenu);$i=$i+2){
			echo '<li><a href="'.$this->aArq[$i].'" alt="'.$this->aMenu[($i + 1)].'">'.$this->aMenu[($i)].'</a>';
			echo '<ul>'.chr(13);
...
/*					  	if ($this->aSubArq[$j] == 'publiquesematbi.php'){
					...
					{ $funcao = $this->funcoesPermitidas->lerRegistro($this->aSubArq[$j+1]);
					  if (($funcao != null) or (($this->aSubArq[$j+1] == 1)and ($this->aSubArq[$j] != 'alterar_senha_sup.php') and ($this->aSubArq[$j] != 'importme1.php')))
  	...
  	public function chamaEstilo(){
		...
		echo '<script type="text/javascript" src="scripts/jquery-ui-1.8.20.custom.min.js"></script>'.chr(13);
 	}
  	...
  	public function chamaCabec(){
            ...
            //$nome_OM = 'om';
	    // PARREIRA - 14-06-2013 - alterado imagem titulo.png para logo_topo25.png e alinhamento de bottom paa middle
            echo '<div class="style1" id="topo">';
            echo '<table width="980" border="0" align="center" cellspacing="2"><tr>';
            echo '	<td width="30%"><div align="left"><img src="imagens/logo_topo25.png" alt=""></div>                        
    ...
    //recebe uma coleção e monta um Combo dos números de BI
    public function montaComboBi($nome, $colecao, $value, $texto, $selected, $onchange, $todos = null) {
        echo '<select name="' . $nome . '" onchange="' . $onchange . '">';
        $obj = $colecao->iniciaBusca1();
        echo '<option value ="">';
        if ($todos != null) {
	...
	*/
	public function retornaStatus($op){
            switch ($op) {
                case 'N':
                    return '<img src="imagens/notanova.png" alt="">';
                    break;
                case 'E':
