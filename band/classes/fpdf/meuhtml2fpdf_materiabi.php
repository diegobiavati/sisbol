...
class meuhtml2fpdf_materiabi extends FPDF
{
//internal attributes
var $HREF; //! string
var $pgwidth; //! float
...
//---Código inserido
  ...
  private $tipoBolDescr;
  private $nrBi;
  private $dataMateriaBi;
  private $biRef;
  private $pGradAssina;
  ...
  public function meuhtml2fpdf_materiabi($orientation='P',$unit='mm',$format='a4', $materiaBi, $tipoBol, 
  							$pGradAssina, $militarAssina, $funcaoAssina, $om, $leftMargim, $siglaSubun)
    { FPDF::FPDF($orientation, $unit, $format);
    $this->tipoBolAbrev = $tipoBol->getAbreviatura();
    $this->tipoBolDescr = $tipoBol->getDescricao();
    $this->materiaBi = $materiaBi;
    $this->dataMateriaBi = $materiaBi->getData;
//	if ($tipoBol->getE_Aditamento() == 'S')
//	    $this->biRef = ' '.$boletim->getBiRef();
//	else
//	    $this->biRef = '';
...
//    $this->om = $om->getSigla();
    $this->res = stristr($this->tipoBolDescr,'reservado');
    $this->leftMargin = $leftMargin;
    $this->siglaSubun = $siglaSubun;
  //}
...
function Header($content='')
{
    ...
    if ($this->res){
        ...
        $this->SetTextColor(215,0,0);
        $this->SetDrawColor(215,0,0);
    ...
    if ($this->PageNo() <> 1)
    {
        ...
        $this->Cell(150,5,'(Continuação da Nota p/ ' . $this->tipoBolAbrev . ' nº ' . $this->materiaBi->getCodigo() . ', de ' . $this->materiaBi->getData()->GetcDataDDBMMBYYYY() .
					', do(a) ' . $this->siglaSubun . ')',0,0,'L');
    ...
    if ($this->PageNo() <> 1)
	   {
	    //Vai para 1.5 cm da borda superior
    	$this->SetY(6);
	    $this->Cell(150,5,'(Continuação do ' .  $this->tipoBol . ' nº ' . $this->nrBi . $this->biRef .', de ' . $this->dataBi .
					', do(a) ' . $this->om . ')',0,0,'L');
	    $this->Cell(30,5,'Pag nº ' . ($this->PageNo()+ $this->pagInicial),0,0,'R');
        $this->SetY(18);
    }else{
...
//    $this->Cell(180,4,'Publicado no '.$this->tipoBolDescr.' nº ______, de ____/_______/____, item _______',0,1,'L'); 
//    $this->Cell(180,4,'-------------------------------------------------------------------------------------------------------------------------------',0,1,'L'); 
...
/// HTML parser ///
///////////////////
function WriteHTML($html,$leftMargin=20.5)
{
//! @desc HTML parser
//! @return void
/* $e == content */
	...
	foreach($a as $i => $e)
	{
        ...
        $tag=strtoupper($a2[0]);
				...
				if (!empty($contents))
				{
  				...
  				{
  				    // Alterado por Ten S.Lopes -- 14/03/2012 -- código anterior = "if(ereg('^([^=]*)=["\']?([^"\']*)["\']?$',$v,$a3)" ereg Depreciado na Versão do PHP 5.3
					if(preg_match('/^([^=]*)=["\']?([^"\']*)["\']?$/', $v, $a3))
    					{
			  ...
			  case 'FONT-SIZE': //Does not support: smaller, larger
                 ...
                 case 'X-SMALL': $this->SetFontSize( (0.77) * 11);
			           ...
			           case 'SMALL': $this->SetFontSize( (0.86)* 11);
  			             break;
  			         case 'MEDIUM': $this->SetFontSize(12);
  			             break;
