...
class meuhtml2fpdfalt extends FPDF
{
//internal attributes
var $HREF; //! string
var $pgwidth; //! float
...
//---CÛdigo inserido
  ...
  public function meuhtml2fpdfalt($orientation='P',$unit='mm',$format='A4')
...
//! @return An object (a class instance)
  ...
  $this->DisplayPreferences('');
	...
	$this->SetFont('Times','',12); // Changeable?(not yet...)
  $this->lineheight = 5; // Related to FontSizePt == 11
  $this->pgwidth = $this->fw - ($this->lMargin + $this->rMargin) - 15 ;
...
//	$codOM = $this->om->getCodOM();
  ...
  */
	...
	if ($this->pagina == 1)
	{
	   ...
	   $this->setX(120);	
       $this->Cell(77,5,'FOLHA DE ALTERA«’ES',0,1,'L');
       $this->Cell($largura,5,strtoupper($this->om->getSubd2()),0,0,'C');
	   $this->setX(120);	
//       $this->Cell(77,5,'GUARNI«√O: '.$this->om->getGu(),0,1,'L');
       $this->Cell(77,5,'GUARNI«√O: '.$this->omVinc->getGu(),0,1,'L');
       if ($this->om->getSubd3() <> '')
       { $this->Cell($largura,5,strtoupper($this->om->getSubd3()),0,0,'L');
         $this->setX(120);	
         $this->Cell(77,5,'FOLHA N∫ '.$this->pagina,0,1,'L');
  /*       if ($this->om->getSubd4() <> ''){
   ...
   */
         $this->Cell($largura,5,$siglaCodOM,0,0,'C');
             ...
             $this->Cell($largura,5,$siglaCodOM,0,0,'C');
	     $this->setX(120);	
       	 $this->Cell($largura,5,'FOLHA N∫ '.$this->pagina,0,1,'L');
       }
	   $this->ln(7);	
	   $this->Cell(16,5,'NOME: ',0,0,'L');
	   //marco
...
//       $this->imprimeNomePDF2($nomeCompleto);
	   //rev 06
       $this->imprimeNomePDF2(trim($this->pessoa->getNome()),trim($this->pessoa->getNomeGuerra()));
	   $this->ln();	
	   $this->Cell(100,5,'POSTO/GRADUA«√O: '.$this->pessoa->getPGrad()->getDescricao(),0,1,'L');
           $qm = strtr($this->pessoa->getQM()->getDescricao(),'·ÈÌÛ˙„ı‚ÍÙÁ‰ÎÔˆ¸','¡…Õ”⁄√’¬ ‘«ƒÀœ÷‹');
           $this->Cell(177,5,'QAS/QMS: '. strtoupper($qm),0,1,'L');	   
           //$this->Cell(177,5,strtoupper($qm),0,1,'L');
	   $idt = strlen($this->pessoa->getIdtMilitar())==10?substr_replace($this->pessoa->getIdtMilitar(), '-', 
	   												strlen($this->pessoa->getIdtMilitar()) - 1, 0)
													:$this->pessoa->getIdtMilitar(); 
       $this->Cell(100,5,'IDENTIDADE: '.$idt,0,0,'L');	//rv 05
//       $this->Cell(100,5,'IDENTIDADE: '.strtoupper($this->pessoa->getIdtMilitar()),0,0,'L');	//rv 05
       if ($auxmes < 7)
       { $this->Cell(77,5,'1∫ Semestre de '. $this->dtInicio->GetIAno(),0,1,'L');
       }
       else
       { $this->Cell(77,5,'2∫ Semestre de '. $this->dtInicio->GetIAno(),0,1,'L');
       }
       $this->Cell(100,5,'CP: '.$this->pessoa->getCP(),0,0,'L');
//       $this->Cell(100,5,'CP: '.'padina:'.$this->pgwidth.'esq:'.$this->lMargin.'dir:'.$this->rMargin,0,0,'L');
//  $this->pgwidth = $this->fw - ($this->lMargin + $this->rMargin) - 10 ;
       $this->Cell(77,5,'PERÕODO: ' . $this->dtInicio->GetcDataDDBMMBYYYY() . ' a ' . $this->dtTermino->GetcDataDDBMMBYYYY(),0,1,'L');
        //fim do cabecalho
  ...
  /*     if ($this->om->getSubd4() == ''){
       ...
       if ($this->om->getSubd5() == ''){
            ...
            $this->Rect(20,20,177,65);
            $this->Rect(20,85,177,204);	
   */
        $this->Rect(20,15,177,53);
        $this->Rect(20,68,177,204);	
...
//       $this->MultiCell(110,5,$this->omVinc->getNome(),1,'J',0);
       $this->Cell(67,5,'FOLHA N∫ '.$this->pagina,0,1,'C');
       $this->Cell(177,5,'',0,1,'L');
       $this->Cell(177,5,'ContinuaÁ„o das Folhas de AlteraÁıes',0,1,'L');
       $this->Cell(round($this->GetStringWidth('do '.$this->pessoa->getPGrad()->getDescricao()))+3,5,'do ' . 
	   				$this->pessoa->getPGrad()->getDescricao() . ' ',0,0,'L');
//       $nomeCompleto = NomeGuerra::setaNomeGuerra($this->pessoa->getNome(),$this->pessoa->getNomeGuerra(),'','');
//       $nomeCompleto = trim($this->pessoa->getNome()) . '(' . trim($this->pessoa->getNomeGuerra()) . ')';
//       $this->imprimeNomePDF2($nomeCompleto);
       $this->imprimeNomePDF2(trim($this->pessoa->getNome()),trim($this->pessoa->getNomeGuerra()));
       ...
       $this->setX(130);	
       if ($auxmes < 7)
       { $this->Cell(67,5,'1∫ Semestre de '. $this->dtInicio->GetIAno(),0,1,'C');
       }
       else
       { $this->Cell(67,5,'2∫ Semestre de '. $this->dtInicio->GetIAno(),0,1,'C');
       }
       $this->Cell(110,5,'CP: '.$this->pessoa->getCP(),0,0,'L');
       $this->Cell(67,5,'PERÕODO: ' . $this->dtInicio->GetcDataDDBMMBYYYY() . ' a ' . $this->dtTermino->GetcDataDDBMMBYYYY(),0,1,'L');
       
           $this->ln();
       	 //  $this->Rect(20,20,177,32);
  	 //  $this->Rect(20,52,177,225);
           $this->Rect(20,20,177,32);
  	   $this->Rect(20,52,177,225);
	}
  }

  public function ImprimeNomePDF2($nome,$nomeGuerra)
	{
		//rev 06
		$nome=strtoupper($nome);
	  	$nome = strtr($nome,'·ÈÌÛ˙„ı‚ÍÙÁ‰ÎÔˆ¸','¡…Õ”⁄√’¬ ‘«ƒÀœ÷‹');
		$nomeGuerra=strtoupper($nomeGuerra);
	  	$nomeGuerra = strtr($nomeGuerra,'·ÈÌÛ˙„ı‚ÍÙÁ‰ÎÔˆ¸','¡…Õ”⁄√’¬ ‘«ƒÀœ÷‹');
		$c1 = explode(" ",$nome);
	    ...
	    if ($c2 != 0)
	    {
		  for ($i = 0; $i < count($c1); $i++) {		//percorre todas as palavras do nome
			for ($j = 0; $j < count($c2); $j++){			//percorre todas as palavras do nome de guerra
				//verifica se a palavra do nome È igual a palavra do nome de guerra
				//ou se a letra inicial do nome È igual a palavra do nome de guerra
				if ( ($c1[$i] == $c2[$j]) or ( substr($c1[$i],0,1) == $c2[$j] and $pri==0) ) {
					if (strlen($c2[$j]) == 1){
						//coloca apenas a letra inicial da palavra em negrito
						$this->SetFont($this->nomeFonte,'B',$this->tamFonte);
				        $this->Write(5,substr($c1[$i],0,1));
				        $this->SetFont($this->nomeFonte,'',$this->tamFonte);
						$this->Write(5,substr($c1[$i],1,round($this->GetStringWidth($c1[$i]))).' ');
						$pri=1;
					...
					//verifica se È a ˙ltima palavra do for (palavras do nome de guerra)
					if ($c2[$j+1]==''){
				        $this->SetFont($this->nomeFonte,'',$this->tamFonte);
	...
	foreach($a as $i => $e)
	{
        ...
        $tag=strtoupper($a2[0]);
				...
				if (!empty($contents))
				{
  				...
  				{
  				    // Alterado por Ten S.Lopes -- 19/03/2012 -- cÛdigo anterior =  "if(ereg('^([^=]*)=["\']?([^"\']*)["\']?$',$v,$a3)" ereg Depreciado na Vers„o do PHP 5.3
  				    if(preg_match('/^([^=]*)=["\']?([^"\']*)["\']?$/', $v, $a3))
...
//			die(print_r($tag));	
			...
			if (isset($attr['WIDTH'])) $this->table['w']	= ConvertSize($attr['WIDTH'],$this->pgwidth) - 20;
		...
		case 'TD':
			...
			if (isset($attr['WIDTH'])) $this->cell[$this->row][$this->col]['w'] = ConvertSize($attr['WIDTH'],$this->pgwidth) - 20;
...
//! @desc Prepares buffered text to be printed with FlowingBlock()
    ...
    for($i=0;$i < $array_size; $i++)
    {
      ...
      if(isset($vetor[4]) and $vetor[4] != '') $this->SetFont('Times');
