...
//---Código inserido
  ...
  public function html2fpdf($orientation='P',$unit='mm',$format='a4')
  //, $pagInicial, $tipoBol, $boletim,
  //							$pGradAssina, $militarAssina, $funcaoAssina, $boletim)
    { FPDF::FPDF($orientation, $unit, $format);
    $this->pagInicial = $pagInicial;
    //$this->tipoBol = $tipoBol->getAbreviatura();
...
/*	$this->SetFont('Times','',12);
    $this->SetY(15);
	   if ($this->PageNo() <> 1)
	   {
    	...
    	$this->SetY(6);
	    $this->Cell(150,5,'(Continuação do ' .  $this->tipoBol . ' Nr ' . $this->nrBi . ', de ' . $this->dataBi .
					')',0,0,'L');
	    $this->Cell(30,5,'Pag Nr ' . ($this->PageNo()+ $this->pagInicial),0,0,'R');
        $this->SetY(18);
    }else{
	    $this->Cell(180,5,'Pag Nr ' . ($this->PageNo()+ $this->pagInicial),0,0,'R');
    }
*/
//! @return void
