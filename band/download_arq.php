...
<HEAD><TITLE>Download de Boletim</TITLE>
	...
	var janelaPDF;
	function tipo(){
		window.location.href = "download_arq.php?tipo=" + document.listaBi.tipoarq.value 
														+ "&ano=" + document.listaBi.anos.value
...
<?
		...
		echo '<h3 class="titulo">&nbsp;&nbsp;<img src="./imagens/item_dir.gif">&nbsp;Arquivos dispon&iacute;veis para download</h3>';
		if (isset($_GET['tipo'])){
		...
		echo '<table><tr>';
		echo '<td>Tipo:&nbsp;</td>';
		echo '<td><select name="tipoarq" onchange="tipo()">';
		if ($tipoAtual == 'boletim'){
			echo '<option value="boletim" selected>Boletins</option>';
			echo '<option value="alteracao">Alteração</option></select>';
		}else{
			echo '<option value="boletim">Boletins</option>';
			echo '<option value="alteracao" selected>Alteração</option></select>';
		}
		echo '</td>';
		echo '<td>&nbsp;&nbsp;&nbsp;</td>';
		echo '<td>&nbsp;&nbsp;&nbsp;</td>';
...
?>
<table width="70%" border="0" cellspacing="0" cellppading="0" class="lista"><tr><td>
