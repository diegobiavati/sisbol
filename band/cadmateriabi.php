<? 	session_start();
	...
	if(isset($_GET['idtMilAss'])){
        ...
        //verifica se o usuário logado pode modificar o modelo da nota
        //S - usuário tem permissão para modificar
...
//		}
		...
		if (acao=="Incluir"){
                    if (modModelo==1){
			if (!window.confirm("Nota salva com sucesso! Deseja salvar o texto dessa nota como modelo no repositório de assuntos?")){
                            modelo=0;
			}else {
                            modelo=1;
                        }
...
//		document.getElementById("divBuscador").style.visibility = "hidden";
	...
	function novaMateria(tipoBol){
		if (window.confirm("A edição de uma nova nota vai descartar todas as alterações não salvas.\nDeseja editar uma nova matéria ?")){
...
//		$materiaBi->setUsuario($apresentacao->getUser()); // Rev 05 - Mai2008
  ...
  <form  method="post" name="cadMateriaBI" action="">
    ...
    <table width="890" border="0" cellspacing="2" bgcolor="#F5F5F5"	style="border-top:1pt solid #006633;border-bottom:1pt solid #006633;border-left:1pt solid #006633;border-right:1pt solid #006633;">
      <tr>
        <td colspan="3" align="left"><b>Dados de referência:</b></td>
        <td align="right"><b>Mostrar dados desta Nota como referência no boletim?</b>&nbsp;
      ...
      <tr>
		...
		<!-- Adicionado READONLY no input, bloqueando a edição do campo pelo usuário - Sgt Bedin 24/04/2013-->
      ...
      <tr>
        <td valign="top">Assunto Específ:&nbsp;</td>
      ...
      <tr>
        <td colspan="4"><b>Militar que assina a matéria:</b>
        ...
        <td><!-- Inicio das Tabs-->
		 ...
		 <table border="0"  width="810" class="lista">
		    ...
		    <!--* PARREIRA 07-05-13- Inserido obs. sobre uso do enter
                    <td>Texto de Abertura:<p class="p_obs1">* Utilize Shift+ENTER para quebra de linha - Utilize somente ENTER para parágrafo</p></td>-->
...
//			echo '<script>carregaMilAss(\''.$materiaBi->getMilitarAss()->getIdMilitar().'\',\'Alterar\')</script>';
	 ...
	 ?>
		...
		//jQuery('#acordeon1').accordion(); --> retirada a chamada do accordion, pois não está sendo utilizado -- Ten Watanabe/Ten S.Lopes -- 16/08/12
		//jQuery('#acordeon2').accordion({ --> retirada a chamada do accordion, pois não está sendo utilizado -- Ten Watanabe/Ten S. Lopes -- 16/08/12
    	autoheight: false
		});
		</script>
